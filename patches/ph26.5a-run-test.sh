#!/bin/bash
# PH26.5A: Run parser test on deployed backend

set -e

echo "=== PH26.5A: Testing MIME parser on deployed backend ==="

# Create test script on the backend
cat > /tmp/test-parser.js << 'TESTSCRIPT'
...
try {
  ...
  console.log('textBody preview:', result.textBody.substring(0, 150).replace(/\n/g, '\\n'));
  ...
  const textOK = result.textBody.includes('preuve') || result.textBody.includes('Message');
  ...
  if (hasText && notPlaceholder && hasAttachment) {
    console.log('');
    console.log('SUCCESS: PH26.5A parser fix validated');
    process.exit(0);
  } else {
    console.log('');
...
kubectl cp /tmp/test-parser.js keybuzz-backend-dev/$POD:/app/test-parser.js

# Run the test
echo ""
echo "--- Running test ---"
kubectl exec -n keybuzz-backend-dev $POD -- node /app/test-parser.js

# Cleanup
kubectl exec -n keybuzz-backend-dev $POD -- rm -f /app/test-parser.js
