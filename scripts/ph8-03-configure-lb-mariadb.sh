#!/bin/bash
# PH8-03 - Configure Hetzner Load Balancer for MariaDB
set -e
...
hcloud load-balancer describe ${LB_NAME} --output json | jq -r '.services[] | select(.listen_port == '${LB_PORT}') | "Service: \(.listen_port) -> \(.destination_port) (\(.protocol))"' || echo "[WARN] Could not verify LB configuration"
