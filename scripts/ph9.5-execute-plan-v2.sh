#!/bin/bash
# PH9.5 EXECUTE PLAN V2 - Réparation complète ETCD HA (3 masters)
# Version corrigée avec etcdctl via crictl exec
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/ph9.5-execute-plan-v2.log"
...
echo "=============================================="
echo "PH9.5 EXECUTE PLAN V2 - RÉPARATION ETCD HA"
echo "Date: $(date)"
echo "=============================================="
echo ""
...
# Fonction pour exécuter etcdctl via crictl
run_etcdctl() {
    local MASTER_IP=$1
    shift
    local CMD="$@"
    ssh root@$MASTER_IP "crictl exec \$(crictl ps --name etcd -q 2>/dev/null | head -1) etcdctl --endpoints=https://127.0.0.1:2379 --cacert=/etc/kubernetes/pki/etcd/ca.crt --cert=/etc/kubernetes/pki/etcd/server.crt --key=/etc/kubernetes/pki/etcd/server.key $CMD 2>&1" || echo "etcdctl command failed"
}
...
kubectl get pods -n kube-system | grep -E 'etcd-|kube-apiserver-' | tee "$LOG_DIR/control-plane-before.txt"

echo ""
echo "[INFO] Vérification des certificats peer sur master-02..."
ssh root@$MASTER2 "openssl x509 -in /etc/kubernetes/pki/etcd/peer.crt -noout -text 2>/dev/null | grep -A2 'Subject Alternative Name' || echo 'No peer cert or cert does not exist'" | tee "$LOG_DIR/master02-peer-cert.txt"

echo ""
echo "[INFO] Vérification ETCD actuel sur master-01 via crictl..."
run_etcdctl $MASTER1 "member list" | tee "$LOG_DIR/etcd-members-before.txt"
...
echo "[INFO] Listing des membres ETCD actuels..."
ETCD_MEMBERS=$(run_etcdctl $MASTER1 "member list")
echo "$ETCD_MEMBERS" | tee "$LOG_DIR/etcd-members-list.txt"

# Supprimer les membres orphelins (tout sauf master-01)
echo ""
echo "[INFO] Suppression des membres ETCD orphelins..."

# Extraire et supprimer chaque membre qui n'est pas k8s-master-01
echo "$ETCD_MEMBERS" | while read -r line; do
    if echo "$line" | grep -v "k8s-master-01" | grep -qE "k8s-master-0[23]"; then
        MEMBER_ID=$(echo "$line" | cut -d',' -f1 | tr -d ' ')
        MEMBER_NAME=$(echo "$line" | grep -oE 'k8s-master-0[23]' || echo "unknown")
        echo "[INFO] Suppression du membre orphelin: $MEMBER_NAME (ID: $MEMBER_ID)"
        run_etcdctl $MASTER1 "member remove $MEMBER_ID" || echo "Failed to remove $MEMBER_ID (may already be removed)"
    fi
done

echo ""
echo "[INFO] Membres ETCD après nettoyage..."
run_etcdctl $MASTER1 "member list" | tee "$LOG_DIR/etcd-members-after-cleanup.txt"
...
echo "[master-02] Suppression de admin.conf et autres configs..."
...
ls -la /etc/kubernetes/ 2>/dev/null || echo "/etc/kubernetes is empty or does not exist"
ls -la /var/lib/etcd/ 2>/dev/null || echo "/var/lib/etcd is empty"
PREP_MASTER02_EOF
...
if [ $JOIN_RESULT -ne 0 ]; then
    echo "[WARN] master-02 join returned non-zero exit code: $JOIN_RESULT"
    echo "[INFO] Checking if join partially succeeded..."
fi
...
ls -la /etc/kubernetes/pki/etcd/ || echo "No etcd PKI"

echo ""
echo "=== ETCD manifest IPs ==="
grep -E "advertise|listen-peer" /etc/kubernetes/manifests/etcd.yaml 2>/dev/null | head -10 || echo "No etcd.yaml"
POST_JOIN_02_EOF
...
echo ""
echo "[INFO] Vérification des pods control-plane après join master-02..."
kubectl get pods -n kube-system | grep -E 'etcd-|kube-apiserver-' | tee "$LOG_DIR/control-plane-after-master02.txt"
...
run_etcdctl $MASTER1 "member list" | tee "$LOG_DIR/etcd-after-master02.txt"

echo ""
echo "[INFO] Vérification de la santé ETCD..."
run_etcdctl $MASTER1 "endpoint health" | tee -a "$LOG_DIR/etcd-after-master02.txt"
...
echo "[INFO] etcd-k8s-master-02 status: $ETCD02_STATUS"

if [ "$ETCD02_STATUS" != "Running" ]; then
    echo "[WARN] etcd-k8s-master-02 is not Running"
    echo "[INFO] Checking etcd logs on master-02..."
    kubectl logs -n kube-system etcd-k8s-master-02 --tail=30 2>&1 | tee "$LOG_DIR/etcd-master02-logs.txt" || echo "Could not get logs"
fi
...
echo "[master-03] Suppression de admin.conf et autres configs..."
rm -f /etc/kubernetes/admin.conf /etc/kubernetes/kubelet.conf /etc/kubernetes/controller-manager.conf /etc/kubernetes/scheduler.conf
...
if [ $JOIN_RESULT -ne 0 ]; then
    echo "[WARN] master-03 join returned non-zero exit code: $JOIN_RESULT"
fi
...
run_etcdctl $MASTER1 "member list" | tee "$LOG_DIR/etcd-final.txt"
...
kubectl get secret redis-test-secret -n keybuzz-system 2>&1 | tee "$LOG_DIR/redis-secret.txt" || echo "redis-test-secret NOT FOUND"
...

{
    ...
    echo ""
    echo "**Date:** $(date)"
    echo "**Script:** ph9.5-execute-plan-v2.sh"
    echo ""
    echo "## Résumé"
    echo ""
...
git add scripts/ph9.5-execute-plan-v2.sh 2>/dev/null || true
git commit -m "fix: PH9.5 control-plane repair v2 - ETCD HA restoration" 2>&1 || echo "[WARN] git commit failed"
git push 2>&1 || echo "[WARN] git push failed"

echo ""
echo "=============================================="
echo "✅ PH9.5 EXECUTE PLAN V2 TERMINÉ"
echo "=============================================="
echo ""
echo "Résumé:"
echo "- Nodes Ready: $NODES_READY/8"
