...
echo "=== PH-STUDIO-07A — Build & Deploy DEV ==="

# --- Step 1: Apply migration 005 (ai_generations) ---
echo ""
echo "--- 1. Apply migration 005 (ai_generations) ---"
kubectl delete configmap migration-07a --namespace default 2>/dev/null || true
kubectl create configmap migration-07a \
...
kubectl delete pod migrate-07a --namespace default 2>/dev/null || true

kubectl run migrate-07a --rm -it --restart=Never \
  ...
  --overrides='{
    "spec": {
      ...
      "volumes": [{"name": "sql", "configMap": {"name": "migration-07a"}}]
    }
