...
echo "=== PH-STUDIO-07A.1 — Promote to PROD ==="

# --- Step 1: Apply migration 006 to PROD DB ---
echo ""
echo "--- 1. Apply migration 006 to PROD DB ---"
kubectl delete configmap migration-07a1-prod --namespace default 2>/dev/null || true
kubectl create configmap migration-07a1-prod \
  --from-file=migration.sql="${API_DIR}/src/db/migrations/006-ai-pipeline-tracking.sql" \
  --namespace default

DB_URL_PROD=$(kubectl get secret keybuzz-studio-api-db -n "$NS_API_PROD" -o jsonpath='{.data.DATABASE_URL}' | base64 -d)

kubectl delete pod migrate-07a1-prod --namespace default 2>/dev/null || true

kubectl run migrate-07a1-prod --rm -it --restart=Never \
  --image=postgres:17-alpine \
  --namespace=default \
  --overrides='{
    "spec": {
      "containers": [{
        "name": "migrate-07a1-prod",
        "image": "postgres:17-alpine",
        "command": ["sh", "-c", "psql \"'"${DB_URL_PROD}"'\" -f /sql/migration.sql"],
        "volumeMounts": [{"name": "sql", "mountPath": "/sql"}]
      }],
      "volumes": [{"name": "sql", "configMap": {"name": "migration-07a1-prod"}}]
    }
  }' 2>&1 || echo "Migration pod completed"

echo "PROD migration 006 applied"
