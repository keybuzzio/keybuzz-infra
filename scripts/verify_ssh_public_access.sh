#!/bin/bash
# Verify SSH public access from install-v3 to all rebuildable servers (47 servers)
# PH2-05: SSH Public Verification
# This script verifies that install-v3 can connect via SSH (using its private key) to all servers via their public IPs
...
LOG_FILE="${LOG_DIR}/ssh-public-verification.log"
...
cat > "${LOG_FILE}" << EOF
========================================
SSH Public Access Verification Log
========================================
Started: $(date -Iseconds)
SSH Key: ${SSH_KEY_PRIV}
Target: All rebuildable servers (excluding bastions)
Method: SSH public IP connection via private key
========================================
...
# Verify SSH access to each server
log_info "Starting SSH public access verification..."
echo ""
...
while IFS=',' read -r hostname ip_public ip_private role <&3; do
    ...
    log_info "Verifying: ${hostname} (${ip_public}) - ${role}"
    ...
    if ssh -i "${SSH_KEY_PRIV}" \
        ...
        >> "${LOG_FILE}" 2>&1; then
        log_success "${hostname} (${ip_public}): SSH public access OK"
        SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
    else
        EXIT_CODE=$?
        log_error "${hostname} (${ip_public}): SSH public access FAILED (exit code: ${EXIT_CODE})"
...
echo "" | tee -a "${LOG_FILE}"
echo "========================================" | tee -a "${LOG_FILE}"
echo "SSH Public Access Verification Summary" | tee -a "${LOG_FILE}"
echo "========================================" | tee -a "${LOG_FILE}"
...
if [[ ${FAILED_COUNT} -gt 0 ]]; then
    ...
    log_error "STOPPING PHASE: Cannot proceed to PH2-06/PH2-07 with failed servers"
    log_error "Please resolve SSH access issues before continuing"
    exit 1
else
    log_success "All ${TOTAL_SERVERS} servers verified successfully"
    log_success "SSH public mesh is functional"
    log_success "Ready to proceed to PH2-06 (SSH private IP verification) and PH2-07 (Ansible inventory update)"
    exit 0
fi
