...
echo "=== PH-STUDIO-08 — Feedback Loop & Learning System — Promote PROD ==="

echo ""
echo "--- 1. Apply migration 009 to PROD DB ---"
kubectl delete configmap migration-08-prod --namespace default 2>/dev/null || true
kubectl create configmap migration-08-prod \
  --from-file=migration.sql="${API_DIR}/src/db/migrations/009-feedback-learning.sql" \
  --namespace default

DB_URL=$(kubectl get secret keybuzz-studio-api-db -n "$NS_API" -o jsonpath='{.data.DATABASE_URL}' | base64 -d)

kubectl delete pod migrate-08-prod --namespace default 2>/dev/null || true

kubectl run migrate-08-prod --rm -it --restart=Never \
  --image=postgres:17-alpine \
  --namespace=default \
  --overrides="{
    \"spec\": {
      \"containers\": [{
        \"name\": \"migrate-08-prod\",
        \"image\": \"postgres:17-alpine\",
        \"command\": [\"sh\", \"-c\", \"psql '\${DB_URL}' -f /sql/migration.sql\"],
        \"volumeMounts\": [{\"name\": \"sql\", \"mountPath\": \"/sql\"}]
      }],
      \"volumes\": [{\"name\": \"sql\", \"configMap\": {\"name\": \"migration-08-prod\"}}]
    }
  }" 2>&1 || echo "Migration pod completed"

echo "Migration 009 applied to PROD"
