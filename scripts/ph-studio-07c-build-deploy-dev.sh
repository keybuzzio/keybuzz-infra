...
echo "=== PH-STUDIO-07C — Build & Deploy DEV ==="

# --- Step 1: Apply migration 007 (client intelligence) ---
echo ""
echo "--- 1. Apply migration 007 (client intelligence) ---"
kubectl delete configmap migration-07c --namespace default 2>/dev/null || true
kubectl create configmap migration-07c \
  --from-file=migration.sql="${API_DIR}/src/db/migrations/007-client-intelligence.sql" \
  --namespace default

DB_URL=$(kubectl get secret keybuzz-studio-api-db -n "$NS_API" -o jsonpath='{.data.DATABASE_URL}' | base64 -d)

kubectl delete pod migrate-07c --namespace default 2>/dev/null || true

kubectl run migrate-07c --rm -it --restart=Never \
  ...
  --overrides="{
    \"spec\": {
      ...
      \"volumes\": [{\"name\": \"sql\", \"configMap\": {\"name\": \"migration-07c\"}}]
    }
...
curl -s -w " [HTTP %{http_code}]\n" https://studio-api-dev.keybuzz.io/api/v1/client/health
