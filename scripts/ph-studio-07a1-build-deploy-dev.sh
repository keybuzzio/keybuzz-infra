...
echo "=== PH-STUDIO-07A.1 — Build & Deploy DEV ==="

# --- Step 1: Apply migration 006 (pipeline tracking) ---
echo ""
echo "--- 1. Apply migration 006 (pipeline tracking) ---"
kubectl delete configmap migration-07a1 --namespace default 2>/dev/null || true
kubectl create configmap migration-07a1 \
...
kubectl delete pod migrate-07a1 --namespace default 2>/dev/null || true

kubectl run migrate-07a1 --rm -it --restart=Never \
  ...
  --overrides='{
    "spec": {
      ...
      "volumes": [{"name": "sql", "configMap": {"name": "migration-07a1"}}]
    }
