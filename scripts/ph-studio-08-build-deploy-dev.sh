...
echo "=== PH-STUDIO-08 — Feedback Loop & Learning System — Build & Deploy DEV ==="

echo ""
echo "--- 1. Apply migration 009 (feedback-learning) ---"
kubectl delete configmap migration-08 --namespace default 2>/dev/null || true
kubectl create configmap migration-08 \
...
kubectl delete pod migrate-08 --namespace default 2>/dev/null || true

kubectl run migrate-08 --rm -it --restart=Never \
  ...
  --overrides="{
    \"spec\": {
      ...
      \"volumes\": [{\"name\": \"sql\", \"configMap\": {\"name\": \"migration-08\"}}]
    }
...
docker build --no-cache -t "$REGISTRY/keybuzz-studio-api:$TAG" . 2>&1 | tail -5
