...
LOG_FILE="/opt/keybuzz/logs/ph10-ui/update-images-v103-actions.log"
mkdir -p "$(dirname "$LOG_FILE")"

exec > >(tee -a "$LOG_FILE") 2>&1

echo "=== PH10-UI-FIX-AMAZON-INBOUND-ACTIONS - Update K8s Manifest ==="
echo "Date: $(date)"
echo ""
...
# Commit + push
git add "$DEPLOY_FILE"
git commit -m "chore(admin-dev): bump to v1.0.3-dev (PH10-UI-FIX-AMAZON-INBOUND-ACTIONS)" || echo "Rien à committer"
