#!/bin/bash
# Régénération stricte etcd en IP privées (SAN 10.0.0.100/101/102) + distribution, state=new
set -e

LOG_DIR="/opt/keybuzz/logs/phase9-etcd-sans"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/ph9-etcd-sans-$(date +%Y%m%d-%H%M%S).log"
...
echo "[INFO] Regenerate PKI etcd sur master-01 avec SAN privés"
...
echo "[INFO] Forcer state=new et initial-cluster en IP privées dans les manifests etcd"
